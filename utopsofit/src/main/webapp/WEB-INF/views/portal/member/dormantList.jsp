...
<div class="page-content">
  ...
  <div class="page-header">
    <div>
      <h1>휴면 회원</h1>
      <p class="page-desc">장기 미접속으로 휴면 처리된 회원 목록을 조회합니다.</p>
    </div>
  </div>
  ...
  <table class="code-table" id="dormantTable">
    <thead>
      <tr>
        ...
        <th style="width:130px;">휴면 처리일시</th>
      </tr>
    </thead>
  </table>
...
function loadTable() {
  if (table) { table.destroy(); $('#dormantTable tbody').empty(); }
  rowNum = 0;
  $.get(ctx + '/member/dormant/list/json', {
    ...
    table = $('#dormantTable').DataTable({
      ...
      columns: [
        ...
        { data: 'updatedAt',     className: 'dt-center', render: function(d) { return d ? d.replace('T',' ').substring(0,16) : '-'; } }
      ],
      order: [[8, 'desc']],
      language: {
  ...
  }).fail(function(xhr) { console.error('[휴면회원 오류]', xhr.status); });
}
