...
CREATE TABLE `inquiry` (
    ...
    `member_no`     BIGINT        NOT NULL                 COMMENT '회원 번호 (FK → member.member_no)',
    `title`         VARCHAR(200)  NOT NULL                 COMMENT '문의 제목',
    `content`       TEXT          NOT NULL                 COMMENT '문의 내용',
    `inq_status`    VARCHAR(20)   NOT NULL DEFAULT 'WAIT'  COMMENT '상태 (WAIT:대기중 / DONE:답변완료)',
    ...
    KEY `idx_inquiry_status`  (`inq_status`),
...
VALUES ('INQ_STATUS', '문의 상태', '문의 처리 상태 코드', 'Y', 50, 'system')
...
VALUES
    ('INQ_STATUS', 'WAIT', '대기중',   '답변 대기 중인 문의', 'Y', 1, 'system'),
    ('INQ_STATUS', 'DONE', '답변완료', '답변이 완료된 문의',  'Y', 2, 'system')
ON DUPLICATE KEY UPDATE `code_nm` = VALUES(`code_nm`);

-- -----------------------------------------------------
--  샘플 데이터 (member_no 1~5 가 존재한다고 가정)
-- -----------------------------------------------------
INSERT INTO `inquiry` (`member_no`, `title`, `content`, `inq_status`, `reply_content`, `reply_dt`, `reply_by`, `created_at`) VALUES
