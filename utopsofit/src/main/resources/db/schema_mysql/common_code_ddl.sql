...
('GENDER_CD',   '성별',   'member.gender_cd — 회원 성별',            'Y', 1),
('PAY_TYPE_CD', '결제수단', 'member.pay_method_cd — 결제 유형 구분', 'Y', 4);
...
-- -----------------------------------------------------
INSERT INTO `com_code_grp` (`grp_cd`, `grp_nm`, `grp_desc`, `use_yn`, `sort_ord`) VALUES
('SOCIAL_TYPE_CD',     '소셜로그인유형', 'member.social_cd — 소셜 로그인 플랫폼 구분',                   'Y', 21),
('MEMBERSHIP_TYPE_CD', '회원유형',       'member.membership_cd — 무료/유료개인/단체유료 구분',            'Y', 22),
('SUB_STATUS_CD',      '구독상태',       'member.sub_status_cd — 구독 진행 상태 구분',                    'Y', 23),
('SUB_PLAN_CD',        '구독플랜',       'member.sub_plan_cd — 구독 플랜(주기/유형) 구분',                'Y', 24),
('GROUP_TYPE_CD',      '단체유형',       'member_group.group_cd — 소속 단체 종류 구분',                   'Y', 25),
('CONTRACT_STATUS_CD', '계약상태',       'member_group.contract_status_cd — 단체 계약 진행 상태 구분',    'Y', 26),
('GROUP_ROLE_CD',      '단체역할',       'member.group_role_cd — 단체 내 구성원 역할 구분',               'Y', 27),
('MEMBER_STATUS_CD',   '계정상태',       'member.status_cd — 회원 계정 활성 상태 구분',                   'Y', 28),
('MEMBER_GRADE_CD',    '회원등급',       'member.grade_cd — 회원 등급 구분',                              'Y', 29);
...
('MEMBERSHIP_TYPE_CD', 'GROUP', '단체유료회원', '단체(member_group) 소속, 단체에서 일괄 결제',  'Y', 3);
...
-- -----------------------------------------------------
-- MEMBER_STATUS : 회원 계정 상태 (member.status_cd)
-- -----------------------------------------------------
INSERT INTO `com_code` (`grp_cd`, `code`, `code_nm`, `code_desc`, `use_yn`, `sort_ord`) VALUES
...
-- -----------------------------------------------------
-- MEMBER_GRADE : 회원 등급 (member.grade_cd)
-- -----------------------------------------------------
INSERT INTO `com_code` (`grp_cd`, `code`, `code_nm`, `code_desc`, `use_yn`, `sort_ord`) VALUES
...
INSERT INTO `com_code_grp` (`grp_cd`, `grp_nm`, `grp_desc`, `use_yn`, `sort_ord`) VALUES
('APP_TYPE_CD',   '앱 구분',     'app_version.app_cd   — 안드로이드/iOS 구분',       'Y', 33),
('STORE_TYPE_CD', '스토어 구분', 'app_version.store_cd — 앱 배포 스토어 구분',       'Y', 34);
