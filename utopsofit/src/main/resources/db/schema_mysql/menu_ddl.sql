...
--
-- [테이블 구조]
--   tb_menu          : 메뉴 마스터 (계층형, 자기참조)
--   tb_usr_menu_auth : 사용자별 메뉴 접근 권한
--
-- [실행 순서]
--   1. tb_menu
--   2. tb_usr_menu_auth  (FK → tb_menu, tb_usr)
-- =====================================================
...
-- 1. 메뉴 마스터 (tb_menu)
-- =====================================================
DROP TABLE IF EXISTS `tb_usr_menu_auth`;
...
CREATE TABLE `tb_menu` (
    ...
    CONSTRAINT `fk_menu_parent` FOREIGN KEY (`parent_menu_no`) REFERENCES `tb_menu` (`menu_no`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='메뉴 마스터';


-- =====================================================
-- 2. 사용자별 메뉴 접근 권한 (tb_usr_menu_auth)
-- =====================================================
CREATE TABLE `tb_usr_menu_auth` (
    `usr_id`     VARCHAR(50)      NOT NULL COMMENT '관리자 ID (FK → tb_usr)',
    `menu_no`    BIGINT UNSIGNED  NOT NULL COMMENT '메뉴번호 (FK → tb_menu)',
    ...
    CONSTRAINT `fk_usr_menu_auth_usr`  FOREIGN KEY (`usr_id`)  REFERENCES `tb_usr`  (`usr_id`)  ON DELETE CASCADE,
    CONSTRAINT `fk_usr_menu_auth_menu` FOREIGN KEY (`menu_no`) REFERENCES `tb_menu` (`menu_no`) ON DELETE CASCADE
...
INSERT INTO `tb_menu` (`menu_no`, `parent_menu_no`, `menu_nm`, `menu_url`, `menu_type`, `sort_ord`, `use_yn`, `created_by`) VALUES
...
-- 2depth: 게시판 관리 하위
INSERT INTO `tb_menu` (`menu_no`, `parent_menu_no`, `menu_nm`, `menu_url`, `menu_type`, `sort_ord`, `use_yn`, `created_by`) VALUES
(40, 5, '문의 관리', '/board/tb_inquiry/list', 'LINK', 1, 'Y', 'admin'),
(41, 5, 'FAQ 관리',  '/board/tb_faq/list',     'LINK', 2, 'Y', 'admin'),
(42, 5, '약관 관리', '/board/tb_terms/list',   'LINK', 3, 'Y', 'admin');
