...
-- 회원 DDL (MySQL 8.x)
-- 스키마: utopsoft
-- 설명: 앱 서비스 이용 회원 (관리자 tb_usr 테이블과 별도)
--
-- [회원 유형]
--   FREE      : 무료 회원 — 기본 콘텐츠 이용 가능
--   PAID      : 유료 개인 회원 — 구독 플랜 결제
--   GROUP     : 단체 유료 회원 — 단체(tb_member_group) 소속, 단체가 일괄 결제
--
-- [실행 순서]
--   1. tb_member_group  (단체 — tb_member 보다 먼저 생성)
--   2. tb_member        (회원 — group_no FK → tb_member_group)
-- =====================================================
...
CREATE TABLE `tb_member_group` (
    ...
    `group_cd`          VARCHAR(20)      NOT NULL DEFAULT 'COMPANY'     COMMENT '단체유형 → tb_com_code GROUP_TYPE (SCHOOL/COMPANY/INSTITUTE/ORG)',
    ...
    -- 계약 / 구독
    `contract_status_cd` VARCHAR(20)     NOT NULL DEFAULT 'ACTIVE'      COMMENT '계약상태 → tb_com_code CONTRACT_STATUS (ACTIVE/EXPIRED/CANCELED)',
    ...
    -- 결제
    `pay_method_cd`     VARCHAR(20)      DEFAULT NULL                   COMMENT '결제 수단 → tb_com_code PAY_TYPE (CARD/BANK/KAKAO_PAY 등)',
...
CREATE TABLE `tb_member` (
    ...
    `gender_cd`         CHAR(1)          DEFAULT NULL                   COMMENT '성별 → tb_com_code GENDER (M:남 / F:여)',
    `birth_dt`          DATE             DEFAULT NULL                   COMMENT '생년월일',
    `profile_img_url`   VARCHAR(500)     DEFAULT NULL                   COMMENT '프로필 이미지 URL',

    -- 소셜 로그인
    `social_cd`         VARCHAR(20)      NOT NULL DEFAULT 'NONE'        COMMENT '소셜 로그인 유형 → tb_com_code SOCIAL_TYPE (NONE/GOOGLE/APPLE/KAKAO/NAVER)',
    `social_id`         VARCHAR(200)     DEFAULT NULL                   COMMENT '소셜 로그인 고유 ID',

    -- 회원 유형
    `membership_cd`     VARCHAR(20)      NOT NULL DEFAULT 'FREE'        COMMENT '회원유형 → tb_com_code MEMBERSHIP_TYPE (FREE/PAID/GROUP)',

    -- 구독 정보 (FREE는 NONE, PAID·GROUP은 ACTIVE/EXPIRED 등)
    `sub_status_cd`     VARCHAR(20)      NOT NULL DEFAULT 'NONE'        COMMENT '구독상태 → tb_com_code SUB_STATUS (NONE/ACTIVE/EXPIRED/CANCELED/PAUSED)',
    `sub_plan_cd`       VARCHAR(30)      DEFAULT NULL                   COMMENT '구독 플랜 → tb_com_code SUB_PLAN (MONTHLY/ANNUAL/TRIAL/GROUP_MONTHLY/GROUP_ANNUAL)',
    ...
    `pay_method_cd`     VARCHAR(20)      DEFAULT NULL                   COMMENT '결제 수단 → tb_com_code PAY_TYPE',
    `last_pay_dt`       DATETIME         DEFAULT NULL                   COMMENT '최종 결제 일시',
    `last_pay_amt`      INT UNSIGNED     DEFAULT NULL                   COMMENT '최종 결제 금액 (원)',

    -- 단체 소속 정보 (GROUP 회원만 해당)
    `group_no`          BIGINT UNSIGNED  DEFAULT NULL                   COMMENT '소속 단체번호 (FK → tb_member_group)',
    `group_role_cd`     VARCHAR(20)      DEFAULT NULL                   COMMENT '단체 내 역할 → tb_com_code GROUP_ROLE (ADMIN/MEMBER)',
    `group_join_dt`     DATETIME         DEFAULT NULL                   COMMENT '단체 가입 일시',

    -- 계정 상태 / 등급
    `status_cd`         VARCHAR(20)      NOT NULL DEFAULT 'ACTIVE'      COMMENT '계정상태 → tb_com_code MEMBER_STATUS (ACTIVE/INACTIVE/DORMANT/WITHDRAWN)',
    `grade_cd`          VARCHAR(20)      NOT NULL DEFAULT 'NORMAL'      COMMENT '등급 → tb_com_code MEMBER_GRADE (NORMAL/SILVER/GOLD/VIP)',

    -- 포인트 / 경험치 캐시 (tb_point_bal, tb_exp_bal 테이블과 동기화)
    `tb_point_bal`         INT UNSIGNED     NOT NULL DEFAULT 0             COMMENT '포인트 잔액 캐시 (tb_point_bal.bal_amt 동기화)',
    `exp_total`         INT UNSIGNED     NOT NULL DEFAULT 0             COMMENT '누적 경험치 캐시 (tb_exp_bal.exp_total 동기화)',
    `exp_level`         SMALLINT UNSIGNED NOT NULL DEFAULT 1            COMMENT '현재 레벨 캐시 (tb_exp_bal.exp_level 동기화)',
    ...
    CONSTRAINT `fk_member_group` FOREIGN KEY (`group_no`) REFERENCES `tb_member_group` (`group_no`) ON DELETE SET NULL
...
-- tb_member_group: 단체 샘플
INSERT INTO `tb_member_group`
